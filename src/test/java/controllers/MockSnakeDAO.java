...
import data.BreederAction;
import data.Snake;
import data.SnakeDAO;

public class MockSnakeDAO implements SnakeDAO {
	...
	public Snake getSnakeInRackAndBin(String rack, String bin) {
		Snake snake = null;
		if (rack != null && bin != null) {
			for (Snake s : allSnakes) {
				if (s.getTempRack() != null && s.getTempRack().equals(rack) && s.getTempBin() != null
						&& s.getTempBin().equals(bin)) {
					snake = s;
				} else if (s.getHomeRack() != null && s.getHomeRack().equals(rack) && s.getHomeBin() != null
						&& s.getHomeBin().equals(bin)) {
					snake = s;
				}
			}
		}
